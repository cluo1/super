...
public class RedisUtils {
    ...
    private static long defaultTimeout = Long.valueOf(SpringBeanUtil.getProperty("redis.cache.time.out"));
...
/**
    ...
    */
/**
     * 缓存String数据，默认超时为配置项中设置的时间（s）
     * @param key
     * @param value
     * @return
     *//*

    public static boolean setStringTimeout(String key, String value) {
        return setString(key, value, defaultTimeout);
    }
...
/**
    ...
    public static <T> boolean setList(String key, List<T> list) {
        ...
        try {
            ...
            key = buildKey(key);
            redisTemplate.opsForList().leftPush(key, list.toArray());
        } catch (Exception e) {
            LOG.error(e.getMessage(), e);
        }
        return false;
